...
describe 'Tweetのテスト', js: true do
  ...
  context 'Tweet投稿削除のテスト' do
    ...
    it '自分の投稿したツイートが削除できる' do
      ...
      expect(current_path).to eq '/tweets'
      expect(page).not_to have_content "#{tweet1.content}"
    end
    it '他人の投稿したツイートを削除できない', js: true do
      ...
      expect(current_path).to eq '/tweets'
      expect(page).to have_content "#{tweet1.content}"
      expect(page).not_to have_css '#tweet-1-delete-link'
    end
  end

  context 'TweetCommentのテスト' do
    let!(:user1) { create(:user) }
    let!(:user2) { create(:user) }
    ...
    let!(:tweet_comment1) do
      create(:tweet_comment,
             content: 'い' * rand(1..300),
             tweet_id: tweet1.id,
             user_id: user2.id)
    end
    let!(:tweet_comment2) do
      build(:tweet_comment,
            content: 'う' * rand(1..300))
    end
    ...
    context 'TweetComment新規投稿のテスト' do
      ...
      it '自分の投稿したツイートコメントを削除できる' do
        ...
        expect(current_path).to eq '/tweets/1'
        expect(page).not_to have_content "#{tweet_comment2.content}"
      end
      ...
      it '他ユーザーの投稿したツイートコメントは削除できない' do
        ...
        expect(current_path).to eq '/tweets/1'
        expect(page).to have_content "#{tweet_comment1.content}"
        expect(page).not_to have_css '#tweet-comment-1-delete-link'
      end
    end
  end
end
