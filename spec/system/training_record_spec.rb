require 'rails_helper'
  ...
  describe '筋トレ記録のテスト', js: true do
    ...
    let!(:training_record1){ build(:training_record, weight: rand(0..999), rep: rand(0..999), set: rand(0..999)) }
    let!(:training_record2){ build(:training_record, weight: rand(0..999), rep: rand(0..999), set: rand(0..999)) }
    before do
    ...
    context '筋トレ新規投稿のテスト' do
      ...
      it '（新規）今日のトレーニング詳細ページにてトレーニング記録をつける。', js: true do
        click_on '記録する！'
        click_link '今日のトレーニングを記録する'
      ...
      it '（2回目）今日のトレーニング詳細ページにてトレーニング記録をつける。', js: true do
        click_on '記録する！'
        click_link '今日のトレーニングを記録する'
        select "#{part1.name}", from: 'training_record_part_id'
        select "#{training1.name}", from: 'training_record_training_id'
  ...
  describe '筋トレ記録編集のテスト', js: true do
    let!(:user) { create(:user) }
    let!(:genre) { create(:genre) }
    let!(:part1) { create(:part, name: '胸') }
    let!(:part2) { create(:part, name: '腹') }
    ...
    let!(:training_record1){ create(:training_record, training_id: training1.id, record_id: record.id, weight: rand(0..999), rep: rand(0..999), set: rand(0..999)) }
    let!(:training_record2){ create(:training_record, training_id: training2.id, record_id: record.id, weight: rand(0..999), rep: rand(0..999), set: rand(0..999)) }
    before do
      visit new_user_session_path
      fill_in 'user[email]', with: user.email
      fill_in 'user[password]', with: user.password
    ...
    context '筋トレ記録編集のテスト' do
      ...
      it '今日のトレーニング記録をつけた後、それを編集する' do
        ...
        within "#{training_record1.id}-training-field" do
          select "#{training3.name}"
        end
        ...
        expect(current_path).to eq "/records/1"
        expect(page).to have_content "#{training3.name}"
